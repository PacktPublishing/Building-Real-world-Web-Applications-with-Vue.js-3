...
<template>
  <nav v-if="totalPages > 1" class="flex justify-center my-8">
    ...
    <span class="mr-4">
      <RouterLink :to="${path}${ currentPage - 1}" v-if="!isOnFirstPage"
        >Previous</RouterLink
      >
    ...
    <span class="ml-4">
      <RouterLink :to="${path}${ currentPage + 1}" v-if="!isOnLastPage"
        >Next</RouterLink
      >
      <span v-else class="opacity-50" aria-hidden="true">Next</span>
    </span>
    <span class="ml-4">
      <RouterLink :to="${path}${totalPages - 1}" v-if="!isOnLastPage"
        >Last</RouterLink
      >
