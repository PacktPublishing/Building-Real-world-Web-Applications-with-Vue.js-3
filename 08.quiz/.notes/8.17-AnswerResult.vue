...
const selfCorrect = computed((): boolean => {
  return (
    answers.value.findIndex(
      (answer) =>
        answer.clientId === self.value &&
        answer.answer === answer.currentQuestion.correctAnswer
    ) > -1
  );
});

const selfAnswer = computed((): number => {
  return answers.value.filter((answer) => answer.clientId === self.value)[0]
    .answer;
});
    ...
    <v-card>
      ...
      <v-list>
        ...
        >
          <v-list-item-content>
            <v-list-item-title
              :class="{ 'font-weight-bold': isAnswerCorrect(index) }"
